...
RUN conda install -y -n base python=3.12 pip && \
    ...
    /opt/conda/bin/python -m ipykernel install --sys-prefix --name base --display-name "base" && \
    conda clean -afy && \
    rm -rf /tmp/ban6003-requirements.txt /root/.cache/pip

USER vscode

RUN /opt/conda/bin/python -m ipykernel install --user --name base --display-name "base"
